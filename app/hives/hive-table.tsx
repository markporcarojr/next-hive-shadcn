...
export function HiveTable({
  ...
  // Update the search functionality to filter by hive number
  const handleHiveSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setHiveSearch(value);

    // Apply filter to the hiveNumber column
    table.getColumn("hiveNumber")?.setFilterValue(value);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 w-full max-w-sm">
          ...
          <Input
            id="hive-search"
            type="text"
            placeholder="Search by hive number..."
            className="w-full"
            value={hiveSearch}
            onChange={handleHiveSearchChange}
          />
        </div>
